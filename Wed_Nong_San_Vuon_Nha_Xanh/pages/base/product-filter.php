...
<script>
    ...
    function setUrlPrice() {
        ...
        // Sử dụng URLSearchParams để xây dựng URL một cách sạch sẽ và an toàn hơn
        var urlParams = new URLSearchParams(window.location.search);
        urlParams.set('pricefrom', inputMin);
        urlParams.set('priceto', inputMax);

        // Tạo URL mới
        var newURL = window.location.pathname + '?' + urlParams.toString();

        btnFilter.href = newURL;
        console.log(newURL);
    }


    // Truyền biến PHP vào JavaScript một cách chính xác
    var urlCategory = "<?php echo $url_category; ?>";
    var urlBrand = "<?php echo $url_brand; ?>";
    var urlPrice = "<?php echo $url_price; ?>";
    var urlSort = "<?php echo $url_sort; ?>";

    var baseUrl = "index.php?page=products";

    // Sử dụng URLSearchParams để kết hợp các tham số URL
    var initialParams = new URLSearchParams();
    if (urlCategory) initialParams.set('category', urlCategory);
    if (urlBrand) initialParams.set('brand', urlBrand);
    if (urlPrice) initialParams.set('price', urlPrice);
    if (urlSort) initialParams.set('sort', urlSort);

    var initialUrl = baseUrl + (initialParams.toString() ? '&' + initialParams.toString() : '');
